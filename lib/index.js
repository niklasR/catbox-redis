...
internals.Connection.prototype.start = function (callback) {
    ...
    if (this.settings.readPort && this.settings.readHost) {
        readClient = Redis.createClient(this.settings.readPort, this.settings.readHost, options);
...
internals.Connection.prototype.get = function (key, callback) {
    ...
    const client = this.readClient ? this.readClient : this.client;

    if (!client) {
        return callback(new Error('Connection not started'));
    }

    client.get(this.generateKey(key), (err, result) => {
